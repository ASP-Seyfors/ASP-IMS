...
<div class="section-title">3. LIFECYCLE TRACEABILITY FLOW (CHRONOLOGICAL)</div>`;
  ...
  exportThriveCreates() {
    ...
    let csvContent = 'Ignore this row - generated by ASP IMS App\n' + headers.join(',') + '\n';
  ...
  exportThriveProducts() {
    let updatesMap = new Map();

    // Gather updates and new items from the audited sessions
    this.parsedAuditSessions.forEach(sess => { 
      (sess.updatedItems || []).forEach(item => updatesMap.set(item.ref, item)); 
      (sess.newItems || []).forEach(item => updatesMap.set(item.ref, item));
    });

    if(updatesMap.size === 0) { 
      alert("No Item Updates or New Items found in uploaded logs."); 
      return; 
    }
    
    // Exact headers from the Thrive Bulk Edit Products template
    let headers = ['ID', 'Product Name', 'New Product Name', 'Product Categories', 'New Product Categories', 'Product Description', 'New Product Description'];
    
    let csvContent = 'Ignore this row - generated by ASP IMS App\n' + headers.join(',') + '\n';

    let combinedRefs = new Set([...updatesMap.keys()]);

    combinedRefs.forEach(ref => {
      let row = Array(headers.length).fill('');
      row[1] = ref; // Product Name (Thrive uses this to match if ID is blank)

      // Cross-reference the live DB for the most current description/category
      let dbItem = (typeof DatabaseManager !== 'undefined' && DatabaseManager.db) 
          ? DatabaseManager.db.find(i => (i.sku || i.ref || '').toUpperCase() === ref.toUpperCase()) 
          : null;

      if (dbItem) {
          if (dbItem.category) row[4] = String(dbItem.category).replace(/"/g, '""'); // New Product Categories
          if (dbItem.desc) row[6] = String(dbItem.desc).replace(/[\r\n]+/g, ' ').replace(/"/g, '""'); // New Product Description
      }

      csvContent += row.map(v => `"${v}"`).join(',') + '\n';
    });

    UIManager.triggerShareOrDownload(csvContent, `Thrive_Edit_Products_${SessionManager.sessionDateStr}.csv`, 'text/csv');
  },
  ...
  exportShopifyProducts() {
    ...
    // Shopify requires exact headers for the Products import
    ...
    db.forEach(item => {
      ...
      // LOGIC: If it is marked inactive OR the price is 0, hide it from the web as a Draft.
      ...
      let row = [
        ...
        `"${safeCost}"`,         
        `"shopify"`,             
        `"DENY"` // <--- Forces Out of Stock label instead of disappearing when Qty hits 0
      ];
  ...
  exportShopifyInventory() {
    ...
    // Shopify requires exact headers for the Inventory update
    ...
    db.forEach(item => {
      ...
      // Calculate Available Qty to push to Shopify
      ...
      let row = [
        ...
        `"${ref}"`,              // SKU
        `"PH Warehouse"`,        // Location (Must match your Shopify Location Name exactly)
        `${available}`           // On hand (new)
      ];
