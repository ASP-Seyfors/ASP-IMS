...
<div class="section-title">3. LIFECYCLE TRACEABILITY FLOW (CHRONOLOGICAL)</div>`;
  ...
  exportThriveInventory() {
    ...
    if(stockMap.size === 0) { 
      alert("No Stocktake items found in uploaded logs."); 
      return; 
    }
    
    let headers = ['SKU', 'PH Warehouse Quantity In Stock'];
    let csvContent = 'Ignore this row - generated by ASP Scanner App\n' + headers.join(',') + '\n';
    
    stockMap.forEach((qty, ref) => {
      csvContent += `"${ref}","${qty}"\n`;
    });
    
    UIManager.triggerShareOrDownload(csvContent, `Thrive_Inventory_Update_${Date.now()}.csv`, 'text/csv');
  },
